...
const TIMELINE_LIKES = 'Timeline Likes'
...
async function addLikesHeader(page) {
  ...
  if ($timelineTitle != null &&
    ...
    $likes.addEventListener('click', () => {
      if (!document.title.startsWith(TIMELINE_LIKES)) {
        setTitle(TIMELINE_LIKES)
      }
      window.scrollTo({top: 0})
    })
...
function onTimelineChange($timeline, page) {
  ...
  for (let $item of $timeline.children) {
    ...
    if ($tweet != null) {
      ...
      if (page == LATEST_TWEETS || page == TIMELINE_RETWEETS || page == TIMELINE_LIKES || page == HOME) {
...
function onTitleChange(title) {
  ...
  if ((currentPage == TIMELINE_RETWEETS || currentPage == TIMELINE_LIKES) &&
    ...
    if (currentPage == TIMELINE_RETWEETS) setTitle(TIMELINE_RETWEETS)
    else if (currentPage == TIMELINE_LIKES) setTitle(TIMELINE_LIKES)
    return
  }
  ...
  if (config.retweets == 'separate' || config.likes == 'separate') {
    ...
    document.body.classList.toggle('TimelineLikes', currentPage == TIMELINE_LIKES)
    updateThemeColor()
  }
  ...
  if (config.likes == 'separate' && (currentPage == LATEST_TWEETS || currentPage == TIMELINE_LIKES || currentPage == HOME)) {
    addLikesHeader(currentPage)
  }

  if ((config.retweets != 'ignore' || config.likes != 'ignore' || config.hideWhoToFollowEtc) && (currentPage == LATEST_TWEETS || currentPage == TIMELINE_RETWEETS || currentPage == TIMELINE_LIKES || currentPage == HOME) ||
...
function shouldHideTweet(tweetType, page) {
  ...
  return tweetType != (page == TIMELINE_RETWEETS ? 'RETWEET' : (page == TIMELINE_LIKES ? 'LIKE' : 'TWEET'))
}
